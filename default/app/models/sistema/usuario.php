...
Load::models('sistema/estado_usuario', 'personas/persona', 'sistema/perfil', 'sistema/recurso', 'sistema/recurso_perfil', 'sistema/acceso');
...
class Usuario extends ActiveRecord {
    ...
    // Geneos (JF - 10012014) - Modificación para que tome todos los estados por defecto en la llamada.
    
    public function getListadoUsuario($estado='todos', $order='', $page=0) {
        $columns = 'usuario.*, perfil.perfil, persona.nombre, persona.apellido, estado_usuario.estado_usuario, estado_usuario.descripcion, sucursal.sucursal';
        $join = self::getInnerEstado();
        $join.= 'INNER JOIN perfil ON perfil.id = usuario.perfil_id ';
        $join.= 'INNER JOIN persona ON persona.id = usuario.persona_id ';        
        ...
        /*
         *  De momento eliminamos el ordenamiento por el error que tira al cargar.
         * 
         */
        /*
        $order = $this->get_order($order, 'nombre', array(                        
    ...
    public function getItems($page=null) {
        $conditions = 'usuario.id IS NOT NULL'; 
        if($page) {            
    ...
    public function validarUsuario($usuario, $pass) {
        ...
        foreach ($this->find("conditions: $conditions") as $item) {
            
            if($item->password == md5(sha1($pass))) {
                return $item->id;
            } else {
                return 0;
            }        
